...
describe("content service media reference hook", () => {
  beforeEach(() => {
    ...
    db.run(sql`CREATE TABLE IF NOT EXISTS post (id TEXT PRIMARY KEY, title TEXT NOT NULL, cover_id TEXT, created_at TEXT, updated_at TEXT)`)
  })
  afterEach(() => {
  ...
  test("an adapter throwing does not break the write", async () => {
    const adapter = {
      replaceForDocument: () => { throw new Error("boom") },
      removeDocument: () => {},
    }
    const service = createContentService(db, post, generateTable(post), undefined, undefined, undefined, undefined, adapter)
    const created = await service.create({ title: "A", cover: "img1" })
