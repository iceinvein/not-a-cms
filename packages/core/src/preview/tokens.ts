...
export function createPreviewTokenService(db: AppDatabase) {
  function generate(collection: string, documentId: string, opts: number | GenerateOptions = DEFAULT_TTL_HOURS): PreviewTokenRecord {
    ...
    db.run(sql`INSERT INTO _preview_tokens (id, token, collection, document_id, expires_at, revoked_at, created_at) VALUES (${id}, ${token}, ${collection}, ${documentId}, ${expiresAt}, NULL, ${now.toISOString()})`)
    return { token, collection, document_id: documentId, expires_at: expiresAt }
  }

  function validate(token: string, opts: ValidateOptions = {}): { collection: string; document_id: string } | null {
