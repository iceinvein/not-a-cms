...
import { buildCollaborationConfig, collabServerUrl, contentCollabDocumentId } from "../../src/lib/collaboration"

describe("admin collaboration config", () => {
  test("creates stable document ids per content field", () => {
    expect(contentCollabDocumentId("blog_post", "doc-1", "body")).toBe("content:blog_post:doc-1:body")
  })

  test("derives websocket URLs from API bases", () => {
    expect(collabServerUrl("http://localhost:4321")).toBe("ws://localhost:4321/collab")
    expect(collabServerUrl("https://cms.example.test/base")).toBe("wss://cms.example.test/base/collab")
  })

  test("only builds config for saved documents", () => {
    expect(buildCollaborationConfig({
