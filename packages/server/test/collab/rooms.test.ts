...
const snapshot = [
  ...
  { docName: "content:post:1:body", users: [{ clientId: "b", name: "Sam", color: "#1" }, { clientId: "c", name: "Jo", color: "#2" }] },
...
describe("buildPresenceRooms", () => {
  test("groups by document, dedupes by name, enriches title, skips non-content", async () => {
    ...
    expect(rooms[0]).toMatchObject({ collection: "post", documentId: "1", title: "Title of post/1" })
