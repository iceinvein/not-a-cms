...
type Folder = { id: string; name: string; parentId: string | null; position?: number; color?: string; icon?: string; roles?: string[] }
type TagEntry = { name: string; color: string; count: number; description?: string; group?: string }

async function upload(ctx: E2EContext, filename: string, tags: string[] = []): Promise<MediaRecord> {
...
export async function runVaultPolishSmoke(ctx: E2EContext) {
  ...
  const post = await ctx.apiJson<{ id: string }>("/api/blog_post", {
    method: "POST",
    body: JSON.stringify({ title: `VP Ref ${stamp}`, slug: `vp-ref-${stamp}`, coverImage: referenced.id, status: "draft" }),
  })
  const usageBefore = await ctx.apiJson<{ count: number }>(`/api/media/${referenced.id}/usage`)
  if (usageBefore.count !== 1) throw new Error(`expected usage 1 before delete, got ${usageBefore.count}`)

  const del = await ctx.apiJson<{ deleted: string[] }>("/api/media/delete", {
    method: "POST",
    body: JSON.stringify({ ids: [doomed.id, referenced.id] }),
  })
  if (del.deleted.length !== 2) throw new Error(`expected 2 deleted ids, got ${JSON.stringify(del.deleted)}`)
  const usageAfter = await ctx.apiJson<{ count: number }>(`/api/media/${referenced.id}/usage`)
  if (usageAfter.count !== 0) throw new Error(`expected usage cleared after delete, got ${usageAfter.count}`)
  ...
  details.push(`Spec 1: bulk-deleted 2 assets (incl. a referenced one); reverse-index usage for ${referenced.id} purged (1 -> 0). Draft ${post.id} retained.`)

  // ----- Spec 4: tag description/group + merge -----
  await ctx.apiJson("/api/media/tags/hero", { method: "PATCH", body: JSON.stringify({ description: "Homepage hero shots", group: "Marketing" }) })
  await ctx.apiJson("/api/media/tags/merge", { method: "POST", body: JSON.stringify({ source: "launch", target: "hero" }) })
  ...
  if (tags.data.some((t) => t.name === "launch")) throw new Error("merged source tag 'launch' still present")
  details.push(`Spec 4: set hero description+group=Marketing; merged launch -> hero (hero now on ${hero.count} asset(s)).`)

  // ----- Spec 3: folder color/icon/reorder + subfolder move -----
  const brand = await ctx.apiJson<Folder>("/api/media/folders", { method: "POST", body: JSON.stringify({ name: `Brand ${stamp}` }) })
  const logos = await ctx.apiJson<Folder>("/api/media/folders", { method: "POST", body: JSON.stringify({ name: `Logos ${stamp}`, parentId: brand.id }) })
  await ctx.apiJson(`/api/media/folders/${brand.id}`, { method: "PATCH", body: JSON.stringify({ color: "#6b9bc9", icon: "star" }) })
  await ctx.apiJson("/api/media/move", { method: "POST", body: JSON.stringify({ ids: [keep.id], folderId: logos.id }) })
  ...
  details.push("Spec 3: created Brand/Logos folders, set Brand color=#6b9bc9 + icon=star, moved an asset into the Logos subfolder.")

  // ----- Spec 5: per-folder permissions + context -----
  const restricted = await ctx.apiJson<Folder>(`/api/media/folders/${logos.id}`, { method: "PATCH", body: JSON.stringify({ roles: ["editor"] }) })
  ...
  const context = await ctx.apiJson<{ role: string; roles: { key: string }[] }>("/api/media/context")
  if (context.role !== "admin") throw new Error(`expected admin context role, got ${context.role}`)
  if (!context.roles.some((r) => r.key === "editor")) throw new Error("context did not return the editor role")
  const adminFolders = await ctx.apiJson<{ data: Folder[] }>("/api/media/folders")
  if (!adminFolders.data.some((f) => f.id === logos.id)) throw new Error("admin should still see the restricted folder (bypass)")
  details.push(`Spec 5: restricted Logos to [editor]; /api/media/context -> role=${context.role}; admin bypass still lists the restricted folder.`)
  ...
  await ctx.agent(["eval", "[...document.querySelectorAll('button')].find(b => b.textContent.includes('Manage tags'))?.click()"], { allowFailure: true })
  ...
  details.push("UI: Vault rendered with colored folders + lock-restricted subfolder; admin folder-style panel exposed Color/Icon/Permissions; Manage-tags showed the 'Marketing' group.")
