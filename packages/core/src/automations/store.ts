...
export function createFlowStore(db: AppDatabase) {
  function createFlow(input: CreateFlowInput): Flow {
    ...
    db.run(
      sql`INSERT INTO _flows (id, name, description, trigger, steps, active, created_at, updated_at) VALUES (${id}, ${input.name}, ${input.description ?? null}, ${triggerJson}, ${stepsJson}, ${active}, ${now}, ${now})`,
    )
  ...
  function updateFlow(id: string, data: Partial<CreateFlowInput>): Flow | null {
    ...
    db.run(
      sql`UPDATE _flows SET name = ${merged.name}, description = ${merged.description ?? null}, trigger = ${JSON.stringify(merged.trigger)}, steps = ${JSON.stringify(merged.steps)}, active = ${active}, updated_at = ${now} WHERE id = ${id}`,
    )
    return getFlowById(id)
  }
  ...
  function deleteFlow(id: string): boolean {
    ...
    const runs = db.all(sql`SELECT id FROM _flow_runs WHERE flow_id = ${id}`) as Array<{
      id: string
    }>
  ...
  function createRun(flowId: string, triggerEvent: string, triggerPayload?: string): FlowRun {
    ...
    const now = new Date().toISOString()
    db.run(
      sql`INSERT INTO _flow_runs (id, flow_id, trigger_event, trigger_payload, status, started_at) VALUES (${id}, ${flowId}, ${triggerEvent}, ${triggerPayload ?? null}, ${"running"}, ${now})`,
    )
    return getRun(id)!
  }
  ...
  function completeRun(id: string, status: FlowRunStatus, error?: string): void {
    const now = new Date().toISOString()
    db.run(
      sql`UPDATE _flow_runs SET status = ${status}, finished_at = ${now}, error = ${error ?? null} WHERE id = ${id}`,
    )
  }
  ...
  function recordStep(input: RecordStepInput): FlowRunStep {
    ...
    const finishedAt =
      input.finished_at ?? (input.status !== "running" ? new Date().toISOString() : null)
    db.run(
      sql`INSERT INTO _flow_run_steps (id, run_id, step_id, status, input, output, branch_taken, started_at, finished_at, error) VALUES (${id}, ${input.run_id}, ${input.step_id}, ${input.status}, ${input.input ?? null}, ${input.output ?? null}, ${input.branch_taken ?? null}, ${startedAt}, ${finishedAt}, ${input.error ?? null})`,
    )
  ...
  function listRuns(flowId: string, limit = 50, offset = 0): FlowRun[] {
    const rows = db.all(
      sql`SELECT * FROM _flow_runs WHERE flow_id = ${flowId} ORDER BY started_at DESC LIMIT ${limit} OFFSET ${offset}`,
    )
    return (rows as any[]).map(parseRunRow)
  }

  function listRecentRuns(
    opts: { status?: FlowRunStatus; limit?: number; offset?: number } = {},
  ): FlowRun[] {
    const limit = opts.limit ?? 50
    const offset = opts.offset ?? 0
    const rows = opts.status
      ? db.all(
          sql`SELECT * FROM _flow_runs WHERE status = ${opts.status} ORDER BY started_at DESC, rowid DESC LIMIT ${limit} OFFSET ${offset}`,
        )
      : db.all(
          sql`SELECT * FROM _flow_runs ORDER BY started_at DESC, rowid DESC LIMIT ${limit} OFFSET ${offset}`,
        )
    return (rows as any[]).map(parseRunRow)
  }
  ...
  function purgeOldRuns(retentionDays: number): number {
    ...
    const oldRuns = db.all(sql`SELECT id FROM _flow_runs WHERE started_at < ${cutoff}`) as Array<{
      id: string
    }>
