...
describe("defineCollection", () => {
  ...
  test("throws for invalid collection name 'BlogPost'", () => {
    expect(() => defineCollection({ name: "BlogPost", fields: {} })).toThrow(
      "Collection name must be snake_case",
    )
  })

  test("throws for invalid collection name with spaces", () => {
    expect(() => defineCollection({ name: "blog post", fields: {} })).toThrow(
      "Collection name must be snake_case",
    )
  })

  test("throws for invalid collection name starting with number", () => {
    expect(() => defineCollection({ name: "1blog", fields: {} })).toThrow(
      "Collection name must be snake_case",
    )
  })
