import { Mail, Rss, Save } from "lucide-react"
...
export function ChannelSettings({ apiBase = "", initialSettings }: Props) {
  ...
  if (loading)
    return (
      <LoadingState
        title="Loading channel settings"
        description="Fetching RSS and email controls."
      />
    )
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#18181b] p-5 space-y-4">
          ...
          <TextField
            label="Feed title"
            value={settings["channel.rss.title"]}
            onChange={(value) => update("channel.rss.title", value)}
          />
          <TextField
            label="Description"
            value={settings["channel.rss.description"]}
            onChange={(value) => update("channel.rss.description", value)}
          />
          <div className="grid gap-4 sm:grid-cols-2">
            <TextField
              label="Language"
              value={settings["channel.rss.language"]}
              onChange={(value) => update("channel.rss.language", value)}
            />
            <TextField
              label="Collection"
              value={settings["channel.rss.collection"]}
              onChange={(value) => update("channel.rss.collection", value)}
            />
          </div>
          <TextField
            label="Item path"
            value={settings["channel.rss.itemPath"]}
            onChange={(value) => update("channel.rss.itemPath", value)}
          />
        </div>
        ...
        <div className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#18181b] p-5 space-y-4">
          ...
          <TextField
            ...
            value={settings["channel.email.title"]}
            onChange={(value) => update("channel.email.title", value)}
          />
          <TextField
            label="Preheader"
            value={settings["channel.email.preheader"]}
            onChange={(value) => update("channel.email.preheader", value)}
          />
          <TextField
            label="Footer text"
            value={settings["channel.email.footerText"]}
            onChange={(value) => update("channel.email.footerText", value)}
          />
          <div className="grid gap-4 sm:grid-cols-2">
            <TextField
              label="From name"
              value={settings["channel.email.fromName"]}
              onChange={(value) => update("channel.email.fromName", value)}
            />
            <TextField
              label="Subject prefix"
              value={settings["channel.email.subjectPrefix"]}
              onChange={(value) => update("channel.email.subjectPrefix", value)}
            />
          </div>
        </div>
      </div>
