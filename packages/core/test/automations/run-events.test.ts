...
describe("engine run events", () => {
  ...
  test("executeFlow emits started, a step per executed node, then completed", async () => {
      ...
      steps: [
        {
          id: "c1",
          type: "condition",
          rules: [{ field: "document.ok", operator: "eq", value: true }],
          match: "all",
    ...
    const stepEvents = events.filter(
      (e): e is Extract<RunEvent, { type: "run.step" }> => e.type === "run.step",
    )
  ...
  test("dryRun emits nothing", async () => {
    ...
    const flow = store.createFlow({
      ...
      trigger: { type: "content.created" },
      steps: [
        { id: "a1", type: "action.email", config: { to: "x@y.z", subject: "Hi" }, next: null },
      ],
    })
    const engine = createFlowEngine(store, { onRunEvent: (e) => events.push(e) })
