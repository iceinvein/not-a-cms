...
import { renderTextChildren, resolveBlockComponent } from "../../src/runtime/block-renderer"
...
describe("resolveBlockComponent", () => {
  ...
  test("custom map overrides defaults", () => {
    expect(resolveBlockComponent({ type: "paragraph" }, { paragraph: "CustomParagraph" })).toBe(
      "CustomParagraph",
    )
  })
  ...
  test("custom map extends defaults", () => {
    expect(
      resolveBlockComponent({ type: "pricing-table" }, { "pricing-table": "PricingTable" }),
    ).toBe("PricingTable")
  })
})
...
describe("renderTextChildren", () => {
  ...
  test("renders bold mark", () => {
    expect(renderTextChildren([{ type: "text", value: "bold", marks: ["bold"] }])).toBe(
      "<strong>bold</strong>",
    )
  })

  test("renders italic mark", () => {
    expect(renderTextChildren([{ type: "text", value: "italic", marks: ["italic"] }])).toBe(
      "<em>italic</em>",
    )
  })

  test("renders code mark", () => {
    expect(renderTextChildren([{ type: "text", value: "code", marks: ["code"] }])).toBe(
      "<code>code</code>",
    )
  })
  ...
  test("escapes HTML in text", () => {
    expect(renderTextChildren([{ type: "text", value: "<script>alert(1)</script>" }])).toBe(
