...
describe("AuthorLiving", () => {
  test("matches the production renderer (name + role)", () => {
    ...
    expectBlockParity(<AuthorLiving attrs={full} editable={false} setText={() => {}} />, "author", full)
  })

  test("matches the renderer with name only (role omitted)", () => {
    const nameOnly = { name: "Dana", role: "" }
    expectBlockParity(<AuthorLiving attrs={nameOnly} editable={false} setText={() => {}} />, "author", nameOnly)
  })

  test("editable mode renders contenteditable name/role holes", () => {
    const html = renderToString(<AuthorLiving attrs={{ name: "Dana", role: "Eng" }} editable setText={() => {}} />)
