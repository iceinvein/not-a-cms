...
export async function runAutomationDryRunSmoke(ctx: E2EContext) {
  ...
  const flow = await ctx.apiJson<FlowRecord>("/api/_flows", {
    ...
    body: JSON.stringify({
      ...
      steps: [
        { id: "a1", type: "action.email", config: { to: "editor@demo.test", subject: "New: {{document.title}}" }, next: "a2" },
        { id: "a2", type: "action.create_content", config: { collection: "author", data: { name: "Mirror of {{document.title}}" } }, next: null },
      ],
    }),
  })
  ...
  const hasTestButton = (await ctx.agent([
    "eval",
  ...
  // Open the Test panel.
  await ctx.agent(["eval", `[...document.querySelectorAll('button')].find(b => b.textContent.trim() === 'Test').click()`])
  ...
  const payload = JSON.stringify({ event: "content.created", collection: "blog_post", document: { title: "Hello World" } })
  await ctx.agent([
    "eval",
  ...
  // Run the dry-run.
  await ctx.agent(["eval", `[...document.querySelectorAll('button')].find(b => b.textContent.trim() === 'Run test').click()`])
  await ctx.agent(["wait", "--load", "networkidle"], { allowFailure: true })
  await ctx.agent(["wait", "800"], { allowFailure: true })
  ...
  if (runsAfter.data.length !== runsBefore.data.length) {
    throw new Error(`Dry-run persisted a run (before ${runsBefore.data.length}, after ${runsAfter.data.length})`)
  }
  if (authorsAfter.data.length !== authorsBefore.data.length) {
    throw new Error(`Dry-run created content (authors before ${authorsBefore.data.length}, after ${authorsAfter.data.length})`)
  }
